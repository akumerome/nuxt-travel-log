...
async function onSubmit(event: FormSubmitEvent<Schema>) {
    error.value = null;
    try {
        const inserted = await $csrfFetch(`/api/locations/${route.params.slug}/add`, {
            method: "POST",
            body: event.data,
        });
...
<UForm ref="form" :schema :state class="space-y-3" @submit="onSubmit">
    ...
    <div class="mt-4 grid">
        ...
        <UButton class="justify-self-end" color="neutral" loading-auto type="submit">Add location log
        </UButton>
    </div>
</UForm>
</template>